...
local function worker(user_args)
  ...
  local medium_level_color = args.medium_level_color or beautiful.mute
  ...
  local function get_battery_state(s)
      return string.match(s, '.+: ([%a%s]+), (%d?%d?%d)%%,?(.*)')
  end
  local function update_widget(widget, stdout)
    local charge = 0
    local status
    for s in stdout:gmatch("[^\r\n]+") do
      local cur_status, charge_str, _ = get_battery_state(s)
